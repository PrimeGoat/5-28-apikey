...
|*  but I had to make a judgment call:   *|
\*  Gotta stop coding at some point. :)  */
...
// Processes incoming API response and formats the data
const processData = function(data) {
    ...
    print(sprintf("Height:    %-10s   Weight:     %s", data.appearance.height[0], data.appearance.weight[0]));
...
const matchHero = function(query) {
    ...
    for(id in ids) {
        if(minimatch(ids[id], query, {nocase: true})) {
            matches[id] = ids[id];
        }
    }
    return matches;
...
// More convenient function name for printing to screen
const print = function(text) {
    console.log(text);
}

// Outputs the standard prompt for input.  This is needed in multiple places, so I centralized it.
...
// Are we expecting a name or ID number?
let expecting = "name";

// Start accepting input and parse it
interact.prompt();
interact.on('line', (line) => {
    line = line.trim();
    switch(expecting) {
        case "name":  // We're expecting a superhero name
            if(line == "exit") {
                print('Exiting.');
                process.exit(0);
            }

            matches = matchHero(line);

            // No matches
            if(!Object.keys(matches).length) {
                print(`Could not match any superheroes with query ${line}.  Try again.\n`);
                expecting = "name";
                interact.prompt();
                return;
            }

            // Match found, let's look it up
            if(Object.keys(matches).length == 1) {
                print(`Found a match: ${matches[Object.keys(matches)[0]]}.  Looking them up...\n`);
                lookupHero(Object.keys(matches)[0]);
                expecting = "name";
                return;
            }

            // More than one match found, we will have to narrow it down
            if(Object.keys(matches).length > 1) {
                print(`Found multiple matches:`);
                for(id in matches) {
                    print(`${id}: ${matches[id]}`);
                }
                expecting = "id"; // Next input will be for an id number, not a superhero name

                print("\nWhich superhero are you picking?  Enter their ID number.");
        ...
        case "id": // We're expecting an id number to narrow down the matches that were found
            if(typeof matches[line] == "undefined") {
                ...
                interact.prompt();
                return;
            } else { // Match found, let's look it up
                print(`You have chosen ${matches[line]}.  Looking them up...\n`);
                lookupHero(line);
                expecting = "name";
                return;
            }
